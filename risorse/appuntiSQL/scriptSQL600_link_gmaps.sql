...
SELECT DropGeoTable('tp_600_snap');
CREATE TABLE "tp_600_snap" AS
SELECT f.pk as pk, f.id AS id, ST_Snap(f.geom, g.geom, 0.000001) as geom
FROM tp_600 as f,(SELECT ST_Collect(geom) as geom  FROM tp_600) as g;
SELECT RecoverGeometryColumn('tp_600_snap', 'geom', 4326, 'POLYGON', 'XY');
...
SELECT DropGeoTable('tp_600_link_gmaps');
CREATE TABLE "tp_600_link_gmaps" AS
SELECT t."pk", t."id", f."tp_id",
								  f."Tipologia",
								  f."Indirizzo",
								  f."Coordinate",
								  f."UPL",
								  ...
								  f."indizizzo_link",
								  t."geom"
FROM "tp_600_snap" t, "fontane_indirizzo_link_gmaps" f
WHERE t."id" = f."tp_id";
SELECT RecoverGeometryColumn('tp_600_link_gmaps', 'geom', 4326, 'POLYGON', 'XY');
...
SELECT  "id",st_union(st_boundary (geom)) as geom
FROM "tp_600_link_gmaps";
